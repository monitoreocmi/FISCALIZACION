...
def generar_panel_luxor_centralizado():
    try:
        ...
        # --- LÓGICA CORREGIDA PARA LEER LA LISTA DE COBROS ---
        cobros_db = []
        c_raw = data_suc_cobros_raw if isinstance(data_suc_cobros_raw, list) else []
        for item in c_raw:
            suc = item.get("sucursal", "")
            mes = item.get("mes", "")
            cobros_db.append({
                "clave_busqueda": f"{suc.upper()} ({mes.upper()})",
                "c": item.get("COBRADO", 0), 
                "p": item.get("PERDIDA_PATRIMONIO", 0), 
                "e": item.get("EXCEDENTE", 0)
            })
        ...
        for m in meses_carpetas:
            ...
            for s in sucursales_mes:
                ...
                for cb in cobros_db:
                    if cb.get('clave_busqueda') == s_key: 
            ...
            # Cambio: Se ordena por 't' descendente para mostrar las que tienen MÁS cobros (peor desempeño procesal)
            ...
            <div id="mes-{m}" class="mes-container">
                ...
                <div id="peores-{m}" class="tab-content">
                    ...
                    <div class="audit-grid-full" style="grid-template-columns: 1fr 1fr;">
                        ...
                        <div class="audit-card" style="border-top-color:var(--rojo)"><h3>CRÍTICOS COBROS Y MITIGACION</h3>{gen_rows(bad_c, css="status-fail-red")}</div>
                    </div>
                </div>
            </div>"""

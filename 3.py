...
def ejecutar():
    try:
        ...
        carpetas_meses = [d for d in os.listdir(ruta_raiz) if d.upper() in MESES_ES and os.path.isdir(os.path.join(ruta_raiz, d))]

        for carpeta_mes in carpetas_meses:
            mes_key = carpeta_mes.upper()
            ruta_mes = os.path.join(ruta_raiz, carpeta_mes)
            print(f"Procesando mes: {mes_key}")
            
            # Escanear subcarpetas de sucursales dentro de cada mes
            for suc in os.listdir(ruta_mes):
                ...
                if os.path.isdir(p_suc):
                    ...
                    if os.path.exists(archivo_fuente):
                        ...
                        # Extraer valores de la tabla del resumen
                        valores = re.findall(r"<td[^>]*>(.*?)</td>", html, re.IGNORECASE | re.DOTALL)
                        
                        if len(valores) >= 3:
                            # Limpiar etiquetas HTML de los valores obtenidos
            ...
            # Generar el index.html local para ese mes específico
            ...
            html_local = f"""<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>
                :root {{ --azul: #002060; --amarillo: #F9D908; --verde: #27ae60; --rojo: #ed1c24; --fondo: #f4f7f6; }}
                ...
                .status-ok {{ color: var(--verde); font-weight: bold; }} .status-fail {{ color: var(--rojo); font-weight: bold; }}
            </style></head><body>
            ...
            with open(os.path.join(ruta_mes, "index.html"), "w", encoding="utf-8") as f:
                f.write(html_local)
            print(f"Index generado en: {carpeta_mes}/index.html")

        # Guardar JSONs globales para el dashboard principal

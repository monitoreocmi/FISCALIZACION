...
def api_tasa_bcv():
    try:
        response = requests.get("https://ve.dolarapi.com/v1/cotizaciones", timeout=5)
        if response.status_code == 200:
            datos = response.json()
            tasa = next((item['promedio'] for item in datos if item['moneda'] == 'USD' and item['fuente'] == 'oficial'), 0)
            return jsonify({"status": "success", "promedio": float(tasa)})
    except: pass
    return jsonify({"status": "error", "promedio": 0.0})

@app.route('/calcular_conversion', methods=['POST'])
def calcular_conversion():
    try:
        ...
        if response.status_code == 200:
            ...
            tasa = next((item['promedio'] for item in datos if item['moneda'] == 'USD' and item['fuente'] == 'oficial'), 0)
            if tasa > 0:
                return jsonify({"status": "success", "monto_usd": round(monto_bs / tasa, 2)})
        return jsonify({"status": "error", "message": "No se pudo obtener la tasa"})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)})

# --- RUTAS DE GESTIÓN DE FOTOS TEMPORALES (INTEGRACIÓN) ---
@app.route('/api/listar_uploads', methods=['GET'])
def listar_uploads():
    temp_dir = os.path.join(RUTA_RAIZ, 'uploads')
    if not os.path.exists(temp_dir): os.makedirs(temp_dir, exist_ok=True)
    fotos = [f for f in os.listdir(temp_dir) if f.lower().endswith(('.jpg', '.png', '.jpeg', '.bmp'))]
    return jsonify(fotos)

@app.route('/uploads/<nombre>')
def servir_upload(nombre):
    return send_from_directory(os.path.join(RUTA_RAIZ, 'uploads'), nombre)

@app.route('/api/borrar_upload', methods=['POST'])
def borrar_upload():
    nombre = request.json.get('nombre')
    ruta = os.path.join(RUTA_RAIZ, 'uploads', nombre)
...
def subir_foto():
    ...
    foto.save(os.path.join(temp_dir, foto.filename))
    return "Foto subida", 200
...
USUARIOS = {
    ...
    "cmartinez": {"pw": "2026", "sucursales": ["IPSFA", "GUACARA"]}
}
...
def tarea_sincronizacion_background():
    ...
    try:
        ...
        print(f"[!] Error en sincronización de fondo: {str(e)}")
    finally:
        PROCESO_ACTIVO = False
...
def guardar():
    ...
    try:
        ...
        if not request.files and not request.form.get('SUCURSAL'):
            script_maestro = os.path.join(RUTA_RAIZ, "sincronizar.py")
            if os.path.exists(script_maestro):
                # --- CAMBIO AQUÍ: Disparar en segundo plano ---
                threading.Thread(target=tarea_sincronizacion_background).start()
                return jsonify({"status": "ok", "message": "Sincronización manual iniciada en segundo plano"}), 200
            else:
        ...
        if os.path.exists(path_xlsx):
            # PARCHE DE SEGURIDAD: dtype para ID y FACTURA para evitar errores de tipo
            df = pd.read_excel(path_xlsx, dtype={'ID': str, 'FACTURA': str})
        else:
            df = pd.DataFrame()

        if not df.empty and ide: df = df[df['ID'].astype(str) != ide]
        
        # --- PARCHE DE MONTO QUE ARREGLAMOS ANTES (opcional pero recomendado por si acaso) ---
        monto_limpio = float(str(d.get('MONTO') or 0).replace(',', '.') if str(d.get('MONTO') or 0).replace(',', '.').replace('.', '', 1).isdigit() else 0)
        ...
        nueva = {
            ...
            'OBSERVACIÓN': d.get('OBSERVACION'), 'MONTO $': monto_limpio,
        ...
        try:
            ...
            # --- CAMBIO AQUÍ: Sincronización automática local y remota en SEGUNDO PLANO ---
            script_maestro = os.path.join(RUTA_RAIZ, "sincronizar.py")
            if os.path.exists(script_maestro):
                threading.Thread(target=tarea_sincronizacion_background).start()
        ...
        # Retorna el éxito de inmediato para liberar el navegador del usuario
        return jsonify({"status": "ok"})
...
def listar(mes, sucursal):
    ...
    if ruta and os.path.exists(ruta):
        # Aseguramos consistencia de tipos al listar
        df = pd.read_excel(ruta, dtype={'FACTURA': str, 'ID': str}).fillna("")
...
def borrar():
    ...
    if rx:
        df = pd.read_excel(rx, dtype={'FACTURA': str, 'ID': str})
...
def borrar_foto():
    ...
    rx = buscar_archivo(mes, data['sucursal'])
    if rx:
        df = pd.read_excel(rx, dtype={'FACTURA': str, 'ID': str}).fillna("")
...
.foto-temp-card { background:#fff3cd; padding:10px; border:1px solid #ffeeba; margin-bottom:10px; display:none; }
...
<div class="header">
    ...
    <div id="box-tasa" style="background: var(--verde); color: white; padding: 5px 10px; border-radius: 5px; font-weight: bold; font-size: 12px;">Tasa BCV: Cargando...</div>
...
<div id="crear" class="tab-content active"><div class="card">
    <div id="contenedor-fotos-temp" class="foto-temp-card">
        <h4>📸 FOTOS DETECTADAS (iVMS)</h4>
        <div id="lista-fotos-temp" style="display:flex; gap:10px; overflow-x:auto;"></div>
    </div>
<form id="fM">
...
<div id="b_m" style="display:none; grid-column:span 2; border:1px solid #ddd; padding:10px; border-radius:5px; background:#f9f9f9;">
    <label>CONVERSOR (Bs a $)</label>
    <div style="display:flex; gap:5px; margin-bottom:10px;">
        <input type="text" id="input_bs" placeholder="Monto en Bs." oninput="this.value = this.value.replace(/[^0-9.,]/g, '');">
        <button type="button" onclick="hacerConversion()" style="background:var(--azul); color:white; border:none; padding:5px 10px; cursor:pointer; border-radius:3px;">Convertir</button>
    </div>
    <label>MONTO $</label>
    <input type="number" name="MONTO" id="e_mon" step="0.01">
</div>
...
// Cargar Tasa BCV
fetch('/api/tasa_bcv').then(r=>r.json()).then(d => {
    if(d.status=='success') document.getElementById('box-tasa').innerText = 'Tasa BCV: ' + d.promedio.toFixed(2);
});

// Función conversión
function hacerConversion() {
    const bs = document.getElementById('input_bs').value;
    if(!bs) return alert('Ingrese monto en Bs');
    const fd = new FormData();
    fd.append('monto_bs', bs);
    fetch('/calcular_conversion', {method: 'POST', body: fd})
    .then(r => r.json())
    .then(d => {
        if(d.status == 'success') document.getElementById('e_mon').value = d.monto_usd;
        else alert('Error: ' + d.message);
    });
}

// LÓGICA DE DETECCIÓN Y ASIGNACIÓN AUTOMÁTICA DE FOTOS IVMS
async function refrescarFotosTemp() {
    try {
        const res = await fetch('/api/listar_uploads');
        const fotos = await res.json();
        const cont = document.getElementById('contenedor-fotos-temp');
        const lista = document.getElementById('lista-fotos-temp');
        if(fotos.length > 0) {
            cont.style.display = 'block';
            lista.innerHTML = fotos.map(f => `
                <div style="text-align:center; background:white; padding:5px; border-radius:4px; border:1px solid #ddd;">
                    <img src="/uploads/${f}" style="width:80px; height:80px; object-fit:cover; cursor:pointer; border-radius:3px;" onclick="asignarFotoIVMS('${f}')" title="Haga clic para asignar">
                    <div style="font-size:9px; max-width:80px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${f}</div>
                </div>
            `).join('');
        } else { cont.style.display = 'none'; }
    } catch(e) { console.error("Error al refrescar capturas: ", e); }
}

function asignarFotoIVMS(nombre) {
    if(confirm("¿Quieres asignar esta captura del iVMS como la FOTO DE INCIDENCIA actual?")) {
        const inputFile = document.querySelector('input[name="FOTO_INCIDENCIA_FILE"]');
        fetch(`/uploads/${nombre}`)
            .then(res => res.blob())
            .then(blob => {
                ...
                if(inputFile) {
                    ...
                    alert("Foto asignada al campo 'FOTOS INCIDENCIAS'. Recuerda guardar el registro para confirmar.");
                    fetch('/api/borrar_upload', { 
                        method: 'POST', 
                        headers: { 'Content-Type': 'application/json' }, 
                        body: JSON.stringify({ nombre }) 
                    }).then(refrescarFotosTemp);
                }
            });
    }
}
setInterval(refrescarFotosTemp, 4000);

function irAlPanel() { window.open("http://192.168.77.7:80", '_blank'); }
...
document.getElementById('fM').onsubmit = async (e) => {
    ...
    try {
        ...
        const interval = setInterval(() => {
            if (width < 95) { width += 2; bar.style.width = width + '%'; }
        }, 400);
        ...
        if (r.status === 'ok') {
            bar.style.width = '100%';
            setTimeout(() => { alert("¡Guardado y Sincronizado exitosamente!"); location.reload(); }, 500);
        } else {

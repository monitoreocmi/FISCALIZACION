...
# Configuración de codificación robusta para evitar errores de consola
try:
                    ...
                    for _, r in df_v.iterrows():
                        ...
                        for i, v in enumerate(r['FILA']):
                            ...
                            if i == idx_a['monto']: tds += f"<td>${r['MONTO_CALC']:,.2f}</td>"
                                ...
                                nombres_fotos = v_s.split()
                                links_html = ""
                                for nom_f in nombres_fotos:
                                    r_foto = f"../../FACTURAS/{n_m}/{suc_f}/{nom_f}"
                                    links_html += f"<span class='foto-link' onclick='openModal(\"{r_foto}\")' title='{nom_f}'>📷</span> "
                                tds += f"<td>{links_html}</td>"
